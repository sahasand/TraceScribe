# Claude DOCX Skill - Complete Documentation

> **Complete export of Claude's Word document creation and editing capabilities**  
> Exported: January 2, 2026

---

# Table of Contents

1. [SKILL.md - Main Overview](#skillmd---main-overview)
2. [docx-js.md - Creating New Documents](#docx-jsmd---creating-new-documents)
3. [ooxml.md - Editing Existing Documents](#ooxmlmd---editing-existing-documents)
...
### Workflow
1. **MANDATORY - READ ENTIRE FILE**: Read `docx-js.md` (~500 lines) completely from start to finish.
...
### Workflow
1. **MANDATORY - READ ENTIRE FILE**: Read `ooxml.md` (~600 lines) completely from start to finish.
...
# ooxml.md - Editing Existing Documents
...
- **Match content placement**: Check which cell contains the content in the reference row and place your content in the same cell position
...
Use the Document class from `scripts/document.py` for all tracked changes and comments. It automatically handles infrastructure setup (people.xml, RSIDs, settings.xml, comment files, relationships, content types).
...
**CRITICAL**: Only mark text that actually changes. Keep ALL unchanged text outside `<w:del>`/`<w:ins>` tags.
...
- **Completely rejecting another author's insertion**: Use `revert_insertion()` on the `<w:ins>` element
- **Completely rejecting another author's deletion**: Use `revert_deletion()` on the `<w:del>` element
...
**IMPORTANT**: Use `revert_insertion()` to reject insertions and `revert_deletion()` to restore deletions.
...
**CRITICAL**: The Document class works with a temporary copy at `doc.unpacked_path`. Always copy images to this temp directory.
...
*End of Claude DOCX Skill Documentation*
